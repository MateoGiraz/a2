...
public class MinHeap implements EdgeHeap{
    ...
    public void insert(Edge data) {
        if(elements!=maxElements){
            ...
            while(arr[aux].weight==arr[getParent(aux)].weight && arr[aux].vDest<arr[getParent(aux)].vDest){
                swap(aux,getParent(aux));
                aux=getParent(aux);
            }
        }        
    ...
    private void doSink(int i, int elements){
        ...
        while(hasLeftChild(index) && 2*index < elements){ //if not, theres not right child either.
          ...
          if(arr[index].weight > arr[smallestChildIndex].weight || (arr[index].weight==arr[smallestChildIndex].weight && arr[index].vDest>arr[smallestChildIndex].vDest)) 
